...
<template>
	<view class="categories-all">
		...
		<view class="card" @click="onClick(item.data.id)" :class="item.type" v-for="(item, index) in categoriesList" :key="index">
...
export default {
	...
	methods: {
		...
		onClick(id) {
			switch (id) {
				case -1:
					this._navigateTo('../rank-list/rank-list');
					break;
				case 0:
					this._navigateTo('../special-topics/special-topics');
					break;
			}
		},
